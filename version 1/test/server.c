...
int main() {
    ...
    while (1) {
       ...
       // A------------------------------- ccept a new connection --------------------------------------------------------------------
        new_sockfd = accept(sockfd, (struct sockaddr *)&client_addr, &client_len);// accept() is used to  allow the server to receive data from any client that has establishing a connection (iterative connection-oriented server)
        if (new_sockfd < 0) {
